 ...
 * produces WebP variants at 500/800/1080/1600w alongside the source — or a
 * single native-width variant if the source is narrower than all of those.
 *
 * How it works:
 *   - `import.meta.glob` (eager, URL mode) enumerates every *variant* under
 *     `src/assets/games/*\/concept\/*\/`. The source jpg/png is deliberately
 *     not globbed — see the note on the glob below.
 *   - Variants are grouped by gameSlug → category → stem; each row renders
 *     one `<img>` per stem, with a srcSet across that stem's widths and the
 *     widest variant as the `src` fallback.
 ...
 *
 * Renders nothing if no variants exist for the slug. Variants are gitignored
 * and regenerated by the build, so this is a generated-file dependency: see
 * the `generate-image-variants` plugin in `vite.config.js`.
 */
...
const galleryIndex = buildGalleryIndex(variantModules)

function buildGalleryIndex(variants) {
